...
class BinanceTrader:
    ...
    def position_trading(self, symbol, timeframe='1d', limit=100):
        try:
            ohlcv = self.exc.fetch_ohlcv(symbol, timeframe=timeframe, limit=limit)
            df = pd.DataFrame(ohlcv, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
            ...
            if df['close'].iloc[-1] < df['sma'].iloc[-1]:
                print(f'Price is below the moving average, opening long position')
                position_size = 0.5
                self.buy(symbol, position_size)

            elif df['close'].iloc[-1] > df['sma'].iloc[-1]:
                print(f'Price is above the moving average, opening short position')
                position_size = 0.5
                self.sell(symbol, position_size)

        except Exception as e:
            print(f'Error while executing position trading strategy: {e}')

    def auto_trade(self, symbol, timeframe='1d', limit=100):
        self.auto_trading = True
        while self.auto_trading:
            self.position_trading(symbol, timeframe, limit)
            time.sleep(60)
        ...
        while True:
            ...
            if choice == '1':
                while True:
                    ...
                    print('1. Position trading')
                    strategy_choice = input('Enter the number of your choice: ')
                    
                    if strategy_choice == '1':
                        symbol = input('Enter the symbol of the cryptocurrency you want to apply the position trading strategy to: ')
